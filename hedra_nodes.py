...
class HedraAPITest:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {},
            "optional": {
                "test_endpoints": ("BOOLEAN", {"default": True}),
            }
        }
    ...
    def test_api(self, test_endpoints=True):
        config = HedraConfig()
        ...
        if not api_key or api_key == "your_api_key_here":
            return ("API key not configured. Please set your API key in config.json",)
        
        results = []
        results.append(f"Testing Hedra API Connection")
        results.append(f"API Key length: {len(api_key)}")
        results.append(f"Base URL: {base_url}")
        results.append("-" * 50)
        
        if test_endpoints:
            headers = {
                'x-api-key': api_key,  # Note lowercase 'x'
                'Accept': 'application/json'
            }
            
            # Test the correct endpoints
            test_urls = [
                (f"{base_url}/models", "GET"),
                (f"{base_url}/assets", "GET"),
                (f"{base_url}/generations", "GET"),
            ]
            ...
            for url, method in test_urls:
                try:
                    ...
                    results.append(f"{method} {url}: {response.status_code}")
                    
                    if response.status_code == 200:
                        results.append(f"  Success! Response: {response.text[:100]}...")
                    else:
                        results.append(f"  Error: {response.text}")
                        
                except Exception as e:
                    results.append(f"{method} {url}: Error - {str(e)}")
...
class HedraAPISetup:
    ...
    RETURN_TYPES = ("STRING",)
    RETURN_NAMES = ("setup_info",)
    FUNCTION = "get_setup_info"
    CATEGORY = "Hedra"

    def get_setup_info(self):
        setup_info = """Hedra Character-3 API Setup Instructions:

1. Subscribe to a paid plan at hedra.com
2. Get your API key from your account dashboard
3. Update config.json with your API key:
   {
       "api_key": "your_api_key_here"
   }
4. The API uses credits from your paid account
...
Note: The API uses lowercase 'x-api-key' header for authentication."""
        
        return (setup_info,)

class HedraConfigCheck:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "check_config": ("BOOLEAN", {"default": True}),
            }
        }
    ...
    def check_config(self, check_config):
        if not check_config:
            return ("Config check disabled",)
        
        config = HedraConfig()
        api_key = config.get_api_key()
        
        status = f"""Hedra Configuration Status:
Config file: {config.config_file}
Config exists: {os.path.exists(config.config_file)}
API key configured: {'Yes' if api_key and api_key != 'your_api_key_here' else 'No'}

Expected base URL: https://api.hedra.com/web-app/public

Next steps:
1. Ensure config.json exists and contains your API key
2. Verify you have a paid Hedra plan
"""
...
NODE_CLASS_MAPPINGS = {
    ...
    "HedraAPISetup": HedraAPISetup,
    "HedraConfigCheck": HedraConfigCheck,
    "HedraAPITest": HedraAPITest,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "HedraImageToVideo": "Hedra Image to Video",
    "HedraAPISetup": "Hedra API Setup Info",
    "HedraConfigCheck": "Hedra Config Check",
    "HedraAPITest": "Hedra API Test",
}